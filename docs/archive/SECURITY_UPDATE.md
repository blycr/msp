...
- 客户端 IP 仅使用 `RemoteAddr`
- 不信任 `X-Forwarded-For` / `X-Real-IP`
- 不依赖 HTTPS/反向代理头推断会话安全策略
...
- **CIDR 支持**：支持 IPv4 CIDR（前缀长度 `0-32`）定义 IP 范围
...
1. 检查是否启用 PIN 认证
2. 某些端点（`/api/pin`、`/api/ip`、`/api/config`）不需要 PIN
3. 从请求头 `X-Session-Token` 或 cookie `msp_session` 中获取会话令牌
4. 验证 PIN 正确后签发会话令牌
...
3. **公网环境**：当前默认模式为家庭局域网，不建议直接公网暴露
4. **定期更换 PIN**：建议定期更换 PIN 码以提高安全性

## 注意事项

1. 配置支持热重载，保存后通常 2 秒内生效
2. 如果配置了 IP 白名单，请确保包含您自己的 IP
3. 如果忘记 PIN，可以编辑 `config.json` 文件修改或禁用
4. `trustProxy` 字段仅为配置兼容保留，当前家庭模式不启用代理头取源 IP
