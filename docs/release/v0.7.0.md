...
- **安全功能全面升级**：
  ...
  - 支持 IPv4 CIDR 范围表示法（`/0` 到 `/32`，常见如 `/8`、`/16`、`/24`）
...
- **PIN 认证**：
  ...
  - 支持通过请求头 `X-Session-Token` 或 Cookie `msp_session` 传递
...
- **安全中间件**：
  ...
  - `/api/pin`、`/api/ip`、`/api/config` 端点不需要 PIN（用于初始化与鉴权流程）
  - IP 过滤在 PIN 验证之前执行
...
1. **CIDR 支持**：当前为 IPv4 CIDR（`/0` 到 `/32`），不支持 IPv6
2. **PIN 重试限制**：暂无重试次数限制
3. **多用户支持**：当前仅支持单个 PIN
4. **密码强度**：无密码强度验证
