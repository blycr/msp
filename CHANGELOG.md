...
- **启动体验**：
  - 启动横幅在局域网访问地址下显示二维码（ANSI 黑白块，手机扫码直达播放页）；`127.0.0.1` 本机地址不打印，二维码生成失败不影响启动
- **视频播放进度回归修复**：
...
- **可靠性修复（配置热重载）**：
  - 配置热重载改为 diff-apply：`LogFile` 变更现在真正重开日志文件（此前静默失效）；`Shares` 变更自动触发媒体缓存后台重建；`Port`/转码编码参数变更明确告警"需重启生效"（此前改了没反应）
- **可靠性修复（后台任务生命周期）**：
  - 首次媒体扫描与请求 ctx 解耦：客户端断开不再杀死全量扫描导致重复扫描
  - 后台重建/缓存落盘统一使用服务级 context；优雅关闭时先取消后台任务并等待收尾（5s 兜底），消除退出时缓存写入截断与 `.tmp` 残留
- **可观测性**：
  - 日志三轨（stdlib log / LoggerService / stderr）收敛到标准库 `log/slog`：全部 40 处日志统一走同一 handler，级别/文件/异步缓冲/轮转语义不变
  - 新增 `/healthz` 端点：返回 `{status, db, uptime}`，豁免 PIN 认证
- **DB 不可用显式化**：
  - 新增 `storage.ErrUnavailable` 哨兵错误；数据库不可用时，播放进度/偏好/收藏 API 返回 503 `{"error":"db_unavailable"}`（此前静默返回空数据，用户无感知）
  - 启动时数据库初始化失败输出醒目横幅提示功能降级；前端识别 503 后收藏按钮置灰、进度保存静默跳过
- **前端性能（渲染模型）**：
  - 列表/播放列表渲染改为 keyed diff：翻页/切 tab/搜索/收藏只更新变化的行，不再全量 `innerHTML` 重建
  - 行事件改容器级事件委托，消除每行独立监听；pager 改稳定引用不再每次重建闭包
  - 缩略图重试状态集中管理，diff 移除行时清理定时器，消除僵尸回调
